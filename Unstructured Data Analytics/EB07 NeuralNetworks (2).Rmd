---
title: "Document Classification with Neural Networks"
...
In this exercise, we train a **Neural Network** to classify documents (email messages). The dataset consists of over sixteen hundred email messages which were previously labeled as either "*ham*" (legitimate messages) or "*spam*" (unsolicited commercial email). The emails in this dataset come from the Enron Corporation and were initially released by the Federal Energy Regulatory Commission as part of their investigation into the collapse of the firm.
...
## 4. Train the Model
To train a Neural Network, we set the method of the caret `train()` function to `"nnet"`. Note that we also had to load the `nnet` package which is required for the method. 
...
**Question:** Can you tell why we included the `capture.output()` function here?

## 5. Evaluate the Model
Using the model that we trained, we assign labels to the messages in the `email_test` dataset. 

```{r}
nnet_pred <- predict(nnet_mod, email_test)
head(nnet_pred)
```
...
Let's build the Naïve Bayes model one more time so we can compare both models.
...
**Question:** (1) How does the Neural Network model perform in contrast to the Naïve Bayes model? (2) What else can we do in order to compare the performance of the two models?
